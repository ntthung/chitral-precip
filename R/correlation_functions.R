...
signif_area <- function(cor.dt, dx, dy) {
  ...
  cor.dt[{signif}, list(long, lat,
                        top    = .SD[.(long,      lat + dy), is.na(signif)],
                        bottom = .SD[.(long,      lat - dy), is.na(signif)],
                        left   = .SD[.(long - dx, lat     ), is.na(signif)],
                        right  = .SD[.(long + dx, lat     ), is.na(signif)])]
}
...
plot_cor <- function(cor.list, cor.limits = NULL) {
  ...
  ggplot(rho) +
    ...
    geom_segment(
      aes(long - 0.25, lat + 0.25, xend = long + 0.25, yend = lat + 0.25),
      rhoSignif[{top}],
      size = 0.1) +
    geom_segment(
      aes(long - 0.25, lat - 0.25, xend = long + 0.25, yend = lat - 0.25),
